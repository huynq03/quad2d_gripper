...
# === PHẦN ĐÃ SỬA: Plot 2 đồ thị song song ===
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6)) # 1 hàng, 2 cột

# Plot β and θ trên ax1
ax1.plot(t, np.rad2deg(beta), label=r'$\beta^d$')
ax1.plot(t, np.rad2deg(theta), label=r'$\theta^d$')
# ax1.axhline(y=beta_min_deg, linestyle='--')
ax1.axvline(x=t_pick, linestyle='--')
ax1.set_xlabel('t (s)')
ax1.set_ylabel('angle (deg)')
ax1.set_title('β and θ with β(t) ≥ {:.1f}°'.format(beta_min_deg))
ax1.legend()
ax1.grid(True, alpha=0.5)

# Plot x and z (for reference) trên ax2
ax2.plot(t, x, label=r'$x_q^d$')
ax2.plot(t, z, label=r'$z_q^d$')
ax2.axvline(x=t_pick, linestyle='--')
ax2.set_xlabel('t (s)')
ax2.set_ylabel('position (m)')
ax2.set_title('Desired quadrotor position trajectories')
ax2.legend()
